...
export default function ChatPage() {
  ...
  // Load rooms accessible to this user's role
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-1">
          ...
          {messages.map((msg, idx) => {
            ...
            const sameMinute = prev && new Date(msg.created_at).getTime() - new Date(prev.created_at).getTime() < 60000
            const grouped = sameSender && sameMinute

            return (
              <div key={msg.id} className={`flex items-start gap-2.5 ${grouped ? 'mt-0.5' : 'mt-3'} ${isMe ? 'flex-row-reverse' : ''}`}>
                {!grouped ? (
                  <Avatar
                  ...
                  {!grouped && (
                    <div className={`flex items-baseline gap-2 mb-0.5 ${isMe ? 'flex-row-reverse' : ''}`}>
                      <span className="text-xs font-semibold">
                        {isMe ? 'You' : (sender?.display_name || sender?.username)}
                      </span>
