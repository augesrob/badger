...
export async function POST(req: NextRequest) {
  ...
  const { roomId, allowed_roles, read_only_roles, action, name, description, orderedIds } = body
  ...
  if (action === 'update_read_only') {
    const { error } = await supabaseAdmin.from('chat_rooms').update({ read_only_roles: read_only_roles ?? [] }).eq('id', roomId)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ ok: true })
  }
