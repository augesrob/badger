...
export function PreShiftTable({ doors, onSave }: {
  doors: StagingDoor[]
  ...
  return (
    <div className="bg-[#1a1a1a] border border-[#333] rounded-xl overflow-hidden">
      {/* Header */}
      <div className="grid grid-cols-[70px_1fr_1fr] gap-0 bg-[#111] border-b-2 border-amber-500">
        <div className="px-3 py-2 text-xs font-bold text-amber-500 uppercase">Door #</div>
      ...
      {doors.map((door, i) => {
        ...
        return (
          <div key={door.id}
            className={`grid grid-cols-[70px_1fr_1fr] gap-0 border-b border-white/5 hover:bg-white/[0.02]
              ${isNewDoorGroup ? 'border-t border-t-[#444]' : ''}
              ${door.door_side === 'A' ? 'bg-white/[0.02]' : ''}`}>
            <div className="px-3 py-1.5 text-sm font-bold text-gray-300">{door.door_label}</div>
            <div className="px-1 py-1 border-l border-[#333]">
              <input
                defaultValue={door.in_front || ''}
                placeholder="—"
                onBlur={e => onSave(door.id, 'in_front', e.target.value)}
                className="w-full bg-transparent border border-transparent rounded px-2 py-1 text-sm font-semibold
