...
export function NotificationBell() {
  ...
  const [chatUnreads, setChatUnreads] = useState<ChatUnread[]>([])
  const [open, setOpen]     = useState(false)
  const ref = useRef<HTMLDivElement>(null)

  // Subscribe to in-memory chat unread store — zero egress
  useEffect(() => {
    return chatUnreadStore.subscribe(setChatUnreads)
  }, [])
  ...
  const dbUnreadCount = notifs.filter(n => !n.is_read).length
  const totalUnread = dbUnreadCount + chatUnreads.length

  const markAllRead = async () => {
    if (!profile) return
    chatUnreadStore.clearAll()
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 bg-nav border border-amber-500/30 rounded-xl shadow-2xl w-80 z-[100] overflow-hidden">
          ...
          <div className="max-h-80 overflow-y-auto">
            {/* Chat unreads — from in-memory store, no DB */}
            {chatUnreads.map(u => (
              <a key={`chat-${u.roomId}`} href="/chat"
                onClick={() => chatUnreadStore.clearRoom(u.roomId)}
                className="block px-4 py-3 border-b border-[#222] text-sm bg-amber-500/5 border-l-2 border-l-amber-500 hover:bg-[#1a1a1a] transition-colors">
                <div className="text-white leading-snug">💬 <span className="font-semibold">{u.roomName}</span></div>
                <div className="text-muted text-xs mt-0.5 truncate">{u.preview}</div>
              </a>
            ))}
            {/* DB notifications */}
            {notifs.length === 0 && chatUnreads.length === 0 ? (
